...
class CSSBundler:
    ...
    def init_app(self, app):
        app.jinja_env.globals.update(CSSBUNDLE=self.__bundler)

        self.stylesheet_tag = app.config.get(
            "CSS_BUNDLER_CUSTOM_TAG", '<link rel="stylesheet" href="{stylesheet}">'
        )

        self.dont_use_bundle = app.config.get("CSS_BUNDLER_DONT_USE_BUNDLE", False)

        self.css_files_path = app.config.get(
            "CSS_BUNDLER_CSS_FILES_PATH", "/static/css/"
        )

        self.bundles_folder = app.config.get("CSS_BUNDLER_BUNDLES_FOLDER", "")

        self.bucket_url = app.config.get("CSS_BUNDLER_BUCKET_URL")

        self.use_bucket_url = app.config.get("CSS_BUNDLER_USE_BUCKET_URL", False)

        self.stop_generating = app.config.get("CSS_BUNDLER_STOP_GENERATING", False)

        self.templates_path = app.config.get("CSS_BUNDLER_TEMPLATES_PATH", "templates/")

        self.add_to_bundle_hash = app.config.get(
            "CSS_BUNDLER_ADD_TO_BUNDLE_FILENAME", "make the md5 different"
        )
    ...
    def generate_bundle_filename(self, stylesheets: List[str]) -> str:
        ...
        _hash.update(self.add_to_bundle_hash.encode())

        return _hash.hexdigest()[:12] + ".css"

    def generate_bundled_css(
        self, bundle_filename: str, stylesheets: List[str]
    ) -> None:
        bundled_css = b""
    ...
    def __process_bundling(self, *args, custom_paths: dict = {}):
        stylesheets = self.__prepare_stylesheet(
            custom_paths=custom_paths, stylesheets=args
        )

        if self.dont_use_bundle is True:
            stylesheets = [
                self.stylesheet_tag.format(stylesheet=stylesheet)
                for stylesheet in stylesheets
            ]

            return stylesheets
        
        bundle_filename = self.generate_bundle_filename(stylesheets=stylesheets)

        if self.stop_generating is False:
            self.generate_bundled_css(
                bundle_filename=bundle_filename, stylesheets=stylesheets
            )
        
        if self.bucket_url is not None and self.use_bucket_url is True:
            bundled_stylesheets = f"{self.bucket_url}{bundle_filename}"
        else:
            bundled_stylesheets = f"{self.css_files_path}{self.bundles_folder}{bundle_filename}"
